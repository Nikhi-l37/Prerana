  ...
  if (isComingSoon) {
    return (
      <div className={cardClass}>
        ...
        <div className={`flex flex-col flex-grow ${isMobile ? 'p-5 text-center items-center' : 'p-8 items-start text-left'}`}>
          <span className="inline-block px-4 py-1.5 rounded-full text-xs font-bold mb-3 bg-orange-100 text-orange-600">
            Opening Soon
          </span>
          <h3 className={`font-bold text-gray-800 ${isMobile ? 'text-xl mb-1' : 'text-2xl mb-4'}`}>{name}</h3>
          <p className={`text-gray-600 ${isMobile ? 'text-sm mb-3' : 'text-base mb-6'}`}>{address}</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className={`flex flex-col flex-grow ${isMobile ? 'p-5 text-center items-center' : 'p-8 items-start text-left'}`}>
        ...
        <span className="mt-auto inline-block text-center bg-gradient-to-br from-[#ff6b35] to-[#d84315] text-white py-2.5 px-6 rounded-lg font-bold shadow-[0_4px_15px_rgba(216,67,21,0.3)]">
          {isMobile ? 'View Menu' : 'View Menu & Details'}
        </span>
      </div>
    </Link>
  );
